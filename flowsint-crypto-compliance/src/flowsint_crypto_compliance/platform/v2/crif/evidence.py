"""RFC-0015 Ch.12 — evidence generator for compliance checks."""
...
class EvidenceGenerator:
    """Generate canonical Evidence records from CRIF registry data."""
    ...
    def generate(
        ...
        *,
        ...
        acquisition_method: str = "crif_registry_collection",
        check_type: str | None = None,
    ) -> list[dict[str, Any]]:
        out: list[dict[str, Any]] = []
        for rec in records:
            et = str(rec.get("entity_type") or "RegistryRecord")
            ...
            evidence = Evidence(
                ...
                explain={"crif_stage": "evidence_generator", "provenance": rec.get("provenance")},
            )
            out.append(
                {
                    "id": str(evidence.id),
                    ...
                    "case_ref": case_ref,
                    "check_type": check_type,
                    "registry_source": connector_id,
                    "jurisdiction": (payload or {}).get("jurisdiction"),
                    "compliance_status": (payload or {}).get("status"),
                }
            )
        return out
